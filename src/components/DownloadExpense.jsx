...
const DownloadExpense = () => {
    ...
    const downloadHandler = async (event) => {
        ...
        const response = await fetch(`${import.meta.env.VITE_BE_URL}/expense/download-expense`, {
            headers: {
    ...
    return (
        <div className="text-center md:text-2xl text-1xl md:m-8 m-4">
            <button className="border-2 md:p-2 p-1 border-red-500 rounded-lg hover:bg-red-600 bg-white m-4" onClick={downloadHandler}>Download Expenses</button>
            <ol className='list-decimal list-inside'>
                {urls.map(item => (
                    <li key={item.expenseUrl}><a href={item.expenseUrl}>{item.expenseUrl}</a></li>
                ))}
            </ol>
        </div>
    );
